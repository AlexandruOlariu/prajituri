...
const PRAJITURI = [
  {
    ...
    descriere: "Choux-uri ușoare și crocante, cu inserție surpriză în interior. Disponibile cu cremă fină de fistic, coulis de zmeură, căpșuni proaspeți sau fructe de pădure. Fiecare bucățică ascunde un miez aromat.",
    stele: "★★★★★",
    pret: "~105 lei/kg",
    categorie: "Choux",
    ingrediente: ["Pâte à choux", "Unt 82%", "Ouă bio", "Faină", "Pastă de fistic 100%", "Mascarpone", "Frișcă naturală 35%", "Zmeură / Căpșuni / Fructe de pădure", "Zahăr pudră", "Vanilie bourbon", "Gelatină"],
    preparare: [
  ...
  },
  {
    ...
    tag: "De sezon",
    imagini: [
      "images/tarte-fructe-1.jpeg"
    ],
    descriere: "Tarte mini cu coajă crocantă, cremă de vanilie mătăsoasă și fructe proaspete de sezon — căpșuni, afine și kiwi. Ușoare, colorate și irezistibile.",
    stele: "★★★★★",
    pret: "~85 lei/kg",
    categorie: "Tarte",
    ingrediente: ["Aluat tarte", "Unt 82%", "Zahăr pudră", "Ouă bio", "Lapte", "Amidon", "Vanilie bourbon", "Mascarpone", "Căpșuni proaspete", "Afine", "Kiwi"],
    preparare: [
  ...
  },
  {
    ...
    tag: "Nou",
    imagini: [
      "images/eclere-1.jpeg",
    ],
    descriere: "Eclere clasice franțuzești umplute cu cremă de patisier și acoperite cu glazură fină din ciocolată belgiană neagră 70%. Crusta perfectă, crema mătăsoasă.",
    stele: "★★★★★",
    pret: "~90 lei/kg",
    categorie: "Eclere",
  ...
  },
  {
    ...
    tag: "Clasic",
    imagini: [
      "images/madlene_1.jpeg",
    ],
    descriere: "Madlene pufoase cu cocoașă perfectă, coapte în forme tradiționale franțuzești. Aromate cu lămâie, vanilie sau portocală — o delicatesă simplă și elegantă, perfectă cu ceaiul.",
    stele: "★★★★★",
    pret: "~70 lei/kg",
    categorie: "Prăjituri",
    ingrediente: ["Ouă bio", "Zahăr", "Unt 82%", "Făină", "Praf de copt", "Coajă de lămâie", "Vanilie bourbon", "Miere"],
    preparare: [
  ...
  },
  {
    ...
    descriere: "Platou personalizat cu selecție de choux, eclere, tarte și prăjituri asortate — ideal pentru evenimente, petreceri sau cadou. Compoziția se stabilește împreună cu voi.",
    stele: "★★★★★",
    pret: "La comandă",
    categorie: "Platouri",
    ingrediente: ["Choux cu inserție", "Eclere cu ciocolată belgiană", "Tarte cu fructe proaspete", "Prăjitură cu cacao și frișcă", "Macarons", "Decor floral comestibil"],
    preparare: [
...
document.getElementById('logo-text').innerHTML =
  CONFIG.numeSite.split(' ')[0] + ' <span>' + (CONFIG.numeSite.split(' ')[1] || '') + '</span>';
document.querySelector('title').textContent =
  CONFIG.numeSite + ' — ' + CONFIG.tagline + ' | Prăjituri, Torturi & Macarons';
document.getElementById('hero-title').innerHTML =
  CONFIG.heroTitlu.replace(/\n/g, '<br />').replace(/(o poveste dulce|sweet|special)/gi, '<em>$1</em>');
...
const socialData = [
  {
    cls: 'social-instagram', icon: '📸', name: 'Instagram',
    handle: CONFIG.instagram, url: CONFIG.instagramUrl
  },
  {
    cls: 'social-whatsapp', icon: '💬', name: 'WhatsApp',
    handle: CONFIG.whatsapp, url: CONFIG.whatsappUrl
  },
  {
    cls: 'social-facebook', icon: '📘', name: 'Facebook',
    handle: CONFIG.facebook, url: CONFIG.facebookUrl
  },
  {
    cls: 'social-email', icon: '✉️', name: 'Email',
    handle: CONFIG.email, url: 'mailto:' + CONFIG.email
  },
];
...
sliderTrack.addEventListener('mouseleave', () => { isDown = false; sliderTrack.classList.remove('grabbing'); });
sliderTrack.addEventListener('mouseup', () => { isDown = false; sliderTrack.classList.remove('grabbing'); });
...
function setGallerySlide(i) {
  ...
  galleryDots.querySelectorAll('.modal-gallery-dot')
...
function openModal(index) {
  ...
  modalIng.innerHTML = p.ingrediente.map(i =>
    `<li>${i}</li>`).join('');

  modalSteps.innerHTML = p.preparare.map((s, i) =>
...
let modalTouchX = 0;
modalImg.addEventListener('touchstart', e => { modalTouchX = e.touches[0].clientX; }, { passive: true });
modalImg.addEventListener('touchend', e => {
  if (galleryImages.length < 2) return;
  const diff = modalTouchX - e.changedTouches[0].clientX;
  if (Math.abs(diff) > 40) setGallerySlide(galleryIndex + (diff > 0 ? 1 : -1));
}, { passive: true });
  ...
  "@graph": [
    {
      ...
      "description": "Patiserie artizanală cu prăjituri făcute cu dragoste din cele mai fine ingrediente. Torturi personalizate, macarons parizieni, eclere, cheesecake și tiramisu.",
      "url": pageUrl,
      "telephone": CONFIG.whatsapp,
      "email": CONFIG.email,
      "sameAs": [CONFIG.instagramUrl, CONFIG.facebookUrl],
      "image": "https://images.unsplash.com/photo-1578985545062-69928b1d9587?w=1200&q=85",
